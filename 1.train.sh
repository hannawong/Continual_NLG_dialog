...
python train.py --output_dir=./outputs/dailydialog_multi --model_type=gpt2 \
--model_name_or_path=gpt2 --do_train  \
--do_eval \
--eval_data_file=Health,Work,Politics,Finance \
...
--block_size=80 \
--max_seq=100 \
--BNM_ratio 0.0 --layer 11 \
--seed 0 \
--alpha 0.6 \
--mode=adapter --gradient_accumulation_step=1 --num_train_epochs 5 --per_gpu_train_batch_size 100 \
--EWC F --aug_method=none --BNM F --replay F --lamol F --AGEM F --only F --dataset dailydialog \
--train F --test T
###
'''
