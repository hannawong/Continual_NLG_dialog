...
def generate_thread(domainname,tokenizer,model,args,task_id):
        ...
        #tokenizer = GPT2Tokenizer.from_pretrained("", do_lower_case=True)
        output_test = []
        torch.set_num_threads(1)
        train_dataset = TextSeqDataset(tokenizer, args, file_paths= [domainname], task_id=task_id,task_name=domainname,max_seq=args.length,mode = "gen")
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            context = input_ids.repeat(5,1)#context.unsqueeze(0).repeat(1, 1) ##shape:[5,25] ##[16,80]
            #print(context.shape)
            generated = context
    
            with torch.no_grad():
                for _ in range(60): ##length = 80
                    outputs = model(generated,labels = None,task_id = task_id,s = 400)
                    next_token_logits = outputs[:,-1, :]
                    filtered_logits = top_k_top_p_filtering(next_token_logits, top_k=args.top_k, top_p=args.top_p)  ###[5,50527], 只是大部分都变成了-inf
                    next_token = torch.multinomial(F.softmax(filtered_logits, dim=-1), num_samples=1) ##[1222]重复5次
                    #next_token = torch.argmax(next_token_logits, dim=-1).unsqueeze(1)
                    #print(generated.shape,next_token.shape)
                    generated = torch.cat((generated, next_token), dim=1) ###[5,26]
            ...
            for i in range(len(out) // 5):
                ans = []
                for j in range(5):
                    text = tokenizer.decode(out[i*5+j], clean_up_tokenization_spaces=True) ##只取到<endoftext>之前
                    ...
                    ans.append(text)
                    #print("no")
                output_test.append([ans])

            for step_ in range(args.length):
                input_ids = input_ids.cuda()
                label = label.cuda()
                print(input_ids.shape)
                ...
                next_tokens = torch.multinomial(F.softmax(filtered_logits, dim=-1), num_samples=1)
                next_tokens = torch.argmax(next_token_logits, dim=-1).unsqueeze(0)
                input_ids = torch.cat((input_ids.cuda(), next_tokens), dim=1) ###[5,26]
        ...
        if args.mode == "adapter":
            json.dump(output_test, open("./data/"+domainname+"/result.json",'w'), indent=2)
        elif args.mode == "ctr":
            json.dump(output_test, open("./data/"+domainname+"/result_ctr.json",'w'), indent=2)
        torch.set_num_threads(1)
        return text
...
if __name__ =='__main__':
    ...
    for i in range(n_proc): 
        generate_thread(domains[i],tokenizer,model,args,perm_domain2id[domains[i]])
